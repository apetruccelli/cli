...
func TestApplyMutations(t *testing.T) {
	...
	t.Run("set_scalar", func(t *testing.T) {
		m := map[string]any{}
		if err := applyMutations(nil, m, map[string]string{"name": "new"}, nil, fields); err != nil {
			t.Fatal(err)
		}
		if m["name"] != "new" {
	...
	t.Run("set_unknown_field_errors", func(t *testing.T) {
		err := applyMutations(nil, map[string]any{}, map[string]string{"bad": "x"}, nil, fields)
	...
	t.Run("set_tag_creates_entry", func(t *testing.T) {
		m := map[string]any{}
		if err := applyMutations(nil, m, map[string]string{"labels.env": "prod"}, nil, fields); err != nil {
			t.Fatal(err)
		}
	...
	t.Run("set_tag_no_subkey_errors", func(t *testing.T) {
		err := applyMutations(nil, map[string]any{}, map[string]string{"labels": "v"}, nil, fields)
	...
	t.Run("set_set_field_adds_member", func(t *testing.T) {
		m := map[string]any{}
		if err := applyMutations(nil, m, map[string]string{"modules.CD": ""}, nil, fields); err != nil {
			t.Fatal(err)
		}
	...
	t.Run("set_set_field_no_member_errors", func(t *testing.T) {
		err := applyMutations(nil, map[string]any{}, map[string]string{"modules": ""}, nil, fields)
	...
	t.Run("del_scalar_sets_nil", func(t *testing.T) {
		...
		if err := applyMutations(nil, m, nil, []string{"name"}, fields); err != nil {
			t.Fatal(err)
		}
		if m["name"] != nil {
	...
	t.Run("del_unknown_field_errors", func(t *testing.T) {
		err := applyMutations(nil, map[string]any{}, nil, []string{"bad"}, fields)
		if err == nil || !strings.Contains(err.Error(), "unknown or read-only") {
			t.Fatalf("err = %v, want unknown or read-only", err)
		}
	})
	...
	t.Run("del_tag_removes_entry", func(t *testing.T) {
		...
		if err := applyMutations(nil, m, nil, []string{"labels.env"}, fields); err != nil {
			t.Fatal(err)
		}
	...
	t.Run("del_tag_no_subkey_errors", func(t *testing.T) {
		err := applyMutations(nil, map[string]any{}, nil, []string{"labels"}, fields)
		if err == nil || !strings.Contains(err.Error(), "require a key") {
			t.Fatalf("err = %v, want require a key", err)
		}
	})
	...
	t.Run("del_set_field_removes_member", func(t *testing.T) {
		...
		if err := applyMutations(nil, m, nil, []string{"modules.CD"}, fields); err != nil {
			t.Fatal(err)
		}
		s := getDotPathSlice(m, "modules")
	...
	t.Run("del_set_field_no_member_errors", func(t *testing.T) {
		err := applyMutations(nil, map[string]any{}, nil, []string{"modules"}, fields)
		if err == nil || !strings.Contains(err.Error(), "require a member") {
			t.Fatalf("err = %v, want require a member", err)
		}
	})
