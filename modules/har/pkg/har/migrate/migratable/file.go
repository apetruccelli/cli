...
func (r *File) Migrate(ctx context.Context) error {
	...
	if r.artifactType == types.PYTHON {
		...
		title := fmt.Sprintf("%s (%s)", fileName, sizeutil.GetSize(int64(r.file.Size)))
		...
		pkgJSONBytes, readme, err := npmutils.ExtractPackageJSONAndReadmeFromTarball(tarFileReader)
		...
		payload, pkgName, version, err := npmutils.BuildNpmUploadFromPackageJSON(pkgJSONBytes, readme, tarFileReader)
		if err != nil {
		...
		title := fmt.Sprintf("%s - %s@%s (%s)", r.file.Name, pkgName, version, sizeutil.GetSize(int64(r.file.Size)))
		stat := types.FileStat{
			Name:     r.file.Name,
			Registry: r.srcRegistry,
			Uri:      r.file.Uri,
